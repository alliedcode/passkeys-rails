RSpec.describe MobilePass::ValidateAuthToken do
  let(:call) { described_class.call auth_token: }

  context "with a valid auth_token" do
    let(:agent) { create(:agent) }
    let(:auth_token) { MobilePass::GenerateAuthToken.call(agent:).auth_token }
  ...
  context "with a JWT with an agent_id, but without an expiration" do
    let(:auth_token) { JWT.encode({ agent_id: 123 }, MobilePass.auth_token_secret, MobilePass.auth_token_algorithm) }
  ...
  context "with a JWT with an expiration, but without an agent_id" do
    let(:auth_token) { JWT.encode({ exp: 1.day.from_now.to_i }, MobilePass.auth_token_secret, MobilePass.auth_token_algorithm) }
  ...
  context "with a JWT with an expiration, but an agent_id that doesn't exist" do
    let(:auth_token) { JWT.encode({ exp: 1.day.from_now.to_i, agent_id: 0 }, MobilePass.auth_token_secret, MobilePass.auth_token_algorithm) }
