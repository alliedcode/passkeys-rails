RSpec.describe Passkeys::Rails::RefreshToken do
  let(:call) { described_class.call token: }
  let(:agent) { create(:agent) }
  let(:token) { nil }

  context "with a valid, non-expired token" do
    let(:token) { Passkeys::Rails::GenerateAuthToken.call(agent:).auth_token }
  ...
  context "with a valid, but expired token" do
    let(:token) { Timecop.freeze(1.year.ago) { Passkeys::Rails::GenerateAuthToken.call(agent:).auth_token } }
